...
{
    public function indexAction($page, $db)
    {
        if(!isset($_GET['page'])) {
            $page = 1 ;
        }
        ...
        if($page < 1){
            die('Page "'.$page.'" inexistante.');
        }

        $perPage = 5 /* $this->container->getParameter('per_page') */ ;

        // Pour récupérer la liste de toutes les annonces : on utilise findAll()
        $listArticles = getArticles($page, $perPage, $db);
    	...
    	// Appel du template
        return array('listArticles' => $listArticles, 'nbPages' => $nbPages, 'page' => $page);
    }
